...
A **single-file chat client** for OpenAI-compatible APIs. Two interfaces — web UI and terminal (TUI) — all live in a single `app.py`, with **zero external dependencies (Python standard library only).**

```
LightChatUI/
├── app.py        # Everything is here (2 UIs + API proxy + shared core)
...
- **Two coexisting UIs** — web / TUI share the same core.
...
- **Volatile by design** — the web UI uses `sessionStorage` (cleared when the tab closes); TUI keeps data in memory only.
...
### Environment variables (TUI only)
...
The TUI is not a browser, so these issues don't exist and it **calls the core directly without a proxy**.
